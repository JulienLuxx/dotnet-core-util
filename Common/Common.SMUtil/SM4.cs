...
namespace Common.SMUtil
{
    internal class SM4
    {
        ...
        private void sm4_setkey(long[] SK, byte[] key)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key), "SM4 密钥不能为空 / SM4 key cannot be null");
            }
            if (key.Length != 16)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(key), key.Length,
                    "SM4 密钥长度必须为 16 字节（128 位）/ SM4 key must be exactly 16 bytes (128 bits)");
            }
        ...
        public byte[] sm4_crypt_cbc(Sm4Context ctx, byte[] iv, byte[] input)
        {
            if (iv == null)
            {
                throw new ArgumentNullException(nameof(iv), "CBC 模式 IV 不能为空 / CBC IV cannot be null");
            }
            if (iv.Length != 16)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(iv), iv.Length,
                    "CBC 模式 IV 长度必须为 16 字节（128 位）/ CBC IV must be exactly 16 bytes (128 bits)");
            }
            if (ctx.isPadding && ctx.mode == 1)
            {
                input = padding(input, 1);
            ...
            else
            {
                ...
                while (num > 0)
                {
                    ...
                    Array.Copy(array, num3 * 16, array6, 0, (num > 16) ? 16 : num);
